...
start "monitor-service" cmd /k "cd services\monitor-service && mvn spring-boot:run"
timeout /t 15 /nobreak

echo [2/4] 启动网关服务 (gateway-service) - 端口: 9000
start "gateway-service" cmd /k "cd services\gateway-service && mvn spring-boot:run"
timeout /t 15 /nobreak
...
start "activity-service" cmd /k "cd services\activity-service && mvn spring-boot:run"
